          ...
          <Card className='mb-6 grid gap-0 overflow-hidden p-0 md:grid-cols-6'>
            <figure className='bg-muted relative flex items-center justify-center overflow-hidden md:col-span-2'>
              <img
                ...
                className='h-41.5 w-32.75 object-contain transition-transform duration-300 hover:scale-105'
                alt='Elver Buds X True Wireless Earbuds'
              />
              <Button className='absolute top-2 right-2 size-7 rounded-full' size='icon'>
                <HeartIcon />
              </Button>
            </figure>
                ...
                return (
                  <TimelineItem key={item.id} status='done' className='gap-x-0'>
                    {item.status === 'done' ? (
                      <TimelineDot
                        status='done'
                        className='my-2.5 size-4.5 border-green-600 bg-green-600 [&>.lucide-check]:size-3.5'
                      />
                    ) : (
                      <TimelineDot
                        status='custom'
